...
def upgrade() -> None:
    ...
    if not inspector.has_table('document_extractions'):
        op.create_table(
    ...
    doc_indexes = {index['name'] for index in inspector.get_indexes('document_extractions')} if inspector.has_table('document_extractions') else set()
    if op.f('ix_document_extractions_status') not in doc_indexes:
        op.create_index(op.f('ix_document_extractions_status'), 'document_extractions', ['status'], unique=False)
    if op.f('ix_document_extractions_tenant_id') not in doc_indexes:
        op.create_index(op.f('ix_document_extractions_tenant_id'), 'document_extractions', ['tenant_id'], unique=False)
    if op.f('ix_document_extractions_intake_item_id') not in doc_indexes:
    ...
    if not inspector.has_table('extraction_issues'):
        op.create_table(
            'extraction_issues',
            sa.Column('id', postgresql.UUID(as_uuid=False), nullable=False),
    ...
    issue_indexes = {index['name'] for index in inspector.get_indexes('extraction_issues')} if inspector.has_table('extraction_issues') else set()
    if op.f('ix_extraction_issues_extraction_id') not in issue_indexes:
        op.create_index(op.f('ix_extraction_issues_extraction_id'), 'extraction_issues', ['extraction_id'], unique=False)
    if op.f('ix_extraction_issues_tenant_id') not in issue_indexes:
    ...
    if inspector.has_table('tickets'):
        ...
        if 'extraction_id' not in ticket_columns:
            op.add_column('tickets', sa.Column('extraction_id', postgresql.UUID(as_uuid=False), nullable=True))

        # SQLite cannot reliably add a foreign key constraint after table creation.
        if bind.dialect.name != 'sqlite':
            ticket_fks = {fk['name'] for fk in inspector.get_foreign_keys('tickets') if fk.get('name')}
            fk_name = op.f('fk_tickets_extraction_id_document_extractions')
            if fk_name not in ticket_fks:
                op.create_foreign_key(fk_name, 'tickets', 'document_extractions', ['extraction_id'], ['id'])
