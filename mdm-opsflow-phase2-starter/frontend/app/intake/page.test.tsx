...
describe("Intake replay token observability page", () => {
  ...
  it("resets audit filters to defaults and refreshes with unscoped queries", async () => {
    const fetchMock = vi.spyOn(global, "fetch").mockImplementation((input: RequestInfo | URL) => {
      const url = String(input);
    ...
    await waitFor(() => {
      expect(fetchMock).toHaveBeenCalled();
      expect(screen.getByLabelText("Audit action")).toHaveValue("all");
      expect(screen.getByLabelText("Audit sort")).toHaveValue("-created_at");
      expect(screen.getByLabelText("Trend granularity")).toHaveValue("day");
      expect(screen.getByLabelText("Audit window preset")).toHaveValue("all");
      expect(screen.getByLabelText("Actor user ID")).toHaveValue("");
      expect(screen.getByLabelText("Token ID")).toHaveValue("");
      expect(screen.getByLabelText("Start created at (UTC ISO)")).toHaveValue("");
      expect(screen.getByLabelText("End created at (UTC ISO)")).toHaveValue("");
    });
    ...
    expect(latestAuditUrl).toBeTruthy();
    expect(latestAuditUrl).toContain("sort=-created_at");
    expect(latestAuditUrl).not.toContain("action=");
    expect(latestAuditUrl).not.toContain("actor_user_id=");
    expect(latestAuditUrl).not.toContain("token_id=");
    expect(latestAuditUrl).not.toContain("start_created_at=");
    expect(latestAuditUrl).not.toContain("end_created_at=");

    expect(latestTrendUrl).toBeTruthy();
    expect(latestTrendUrl).toContain("granularity=day");
    expect(latestTrendUrl).not.toContain("action=");
    expect(latestTrendUrl).not.toContain("actor_user_id=");
  ...
  it("renders grouped audit window presets for faster scanning", async () => {
    vi.spyOn(global, "fetch").mockImplementation((input: RequestInfo | URL) => {
      const url = String(input);
    ...
    const presetSelect = screen.getByLabelText("Audit window preset") as HTMLSelectElement;
    const groupLabels = Array.from(presetSelect.querySelectorAll("optgroup")).map((group) => group.label);

    expect(groupLabels).toEqual(["Hours", "Days and months", "Years and long-range"]);
    expect(screen.getByRole("option", { name: "Last 1 year (365 days)" })).toBeInTheDocument();
    expect(screen.getByRole("option", { name: "Custom (manual timestamps)" })).toBeInTheDocument();
  });
  ...
  it("applies the last-1825-days preset when refreshing audit history", async () => {
    const nowSpy = vi.spyOn(Date, "now").mockReturnValue(Date.parse("2026-07-26T00:00:00.000Z"));
    const fetchMock = vi.spyOn(global, "fetch").mockImplementation((input: RequestInfo | URL) => {
      const url = String(input);
    ...
    try {
      ...
      await user.selectOptions(screen.getByLabelText("Audit window preset"), "last_1825d");
      await user.click(screen.getByRole("button", { name: "Refresh audit" }));
      ...
      expect(latestAuditUrl).toBeTruthy();
      expect(latestAuditUrl).toContain("start_created_at=2021-07-27T00%3A00%3A00.000Z");
      expect(latestAuditUrl).toContain("end_created_at=2026-07-26T00%3A00%3A00.000Z");
    } finally {
      nowSpy.mockRestore();
    }
  ...
  it("applies the last-3650-days preset when refreshing audit history", async () => {
    const nowSpy = vi.spyOn(Date, "now").mockReturnValue(Date.parse("2026-07-26T00:00:00.000Z"));
    const fetchMock = vi.spyOn(global, "fetch").mockImplementation((input: RequestInfo | URL) => {
      const url = String(input);
    ...
    try {
      ...
      await user.selectOptions(screen.getByLabelText("Audit window preset"), "last_3650d");
      await user.click(screen.getByRole("button", { name: "Refresh audit" }));
      ...
      expect(latestAuditUrl).toBeTruthy();
      expect(latestAuditUrl).toContain("start_created_at=2016-07-28T00%3A00%3A00.000Z");
      expect(latestAuditUrl).toContain("end_created_at=2026-07-26T00%3A00%3A00.000Z");
    } finally {
      nowSpy.mockRestore();
    }
  ...
  it("applies the last-7300-days preset when refreshing audit history", async () => {
    const nowSpy = vi.spyOn(Date, "now").mockReturnValue(Date.parse("2026-07-26T00:00:00.000Z"));
    const fetchMock = vi.spyOn(global, "fetch").mockImplementation((input: RequestInfo | URL) => {
      const url = String(input);
    ...
    try {
      ...
      await user.selectOptions(screen.getByLabelText("Audit window preset"), "last_7300d");
      await user.click(screen.getByRole("button", { name: "Refresh audit" }));
      ...
      expect(latestAuditUrl).toBeTruthy();
      expect(latestAuditUrl).toContain("start_created_at=2006-07-31T00%3A00%3A00.000Z");
      expect(latestAuditUrl).toContain("end_created_at=2026-07-26T00%3A00%3A00.000Z");
    } finally {
      nowSpy.mockRestore();
    }
  ...
  it("applies the last-10950-days preset when refreshing audit history", async () => {
    const nowSpy = vi.spyOn(Date, "now").mockReturnValue(Date.parse("2026-07-26T00:00:00.000Z"));
    const fetchMock = vi.spyOn(global, "fetch").mockImplementation((input: RequestInfo | URL) => {
      const url = String(input);
    ...
    try {
      ...
      await user.selectOptions(screen.getByLabelText("Audit window preset"), "last_10950d");
      await user.click(screen.getByRole("button", { name: "Refresh audit" }));
      ...
      expect(latestAuditUrl).toBeTruthy();
      expect(latestAuditUrl).toContain("start_created_at=1996-08-02T00%3A00%3A00.000Z");
      expect(latestAuditUrl).toContain("end_created_at=2026-07-26T00%3A00%3A00.000Z");
    } finally {
      nowSpy.mockRestore();
    }
