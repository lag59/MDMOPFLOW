...
export default function ModuleDetailPage() {
  ...
  const renderCompanyOwnerContent = () => {
    ...
    if (detail.roleKey === "project_manager" && detail.moduleSlug === "projects") {
      ...
      const showSimplifiedProjectManagerModule = true;
      if (showSimplifiedProjectManagerModule) {
        return (
          <section className="space-y-6">
            <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
              <h2 className="text-xl font-semibold text-slate-900">Project Operations Command Center</h2>
              <p className="mt-2 text-sm text-slate-600">
                Prioritized view for project managers. Start with exceptions, then assign actions.
              </p>
              <div className="mt-4 flex flex-wrap gap-2">
                <a href="#pm-priority" className="rounded-lg border border-slate-300 px-3 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-100">Priority Queue</a>
                <a href="#pm-projects" className="rounded-lg border border-slate-300 px-3 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-100">Projects</a>
                <a href="#pm-actions" className="rounded-lg border border-slate-300 px-3 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-100">Action Plan</a>
                <Link href="/project-manager" className="rounded-lg border border-orange-300 px-3 py-2 text-sm font-semibold text-orange-700 hover:bg-orange-50">Open Full PM Workspace</Link>
              </div>
            </div>
            ...
            <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
              <div className="rounded-xl border border-slate-200 bg-white p-5">
                ...
                <div className="mt-2 text-3xl font-bold text-slate-900">{ownerSummary.activeProjects}</div>
              </div>
              <div className="rounded-xl border border-red-200 bg-red-50 p-5">
                <div className="text-xs font-semibold uppercase tracking-wide text-red-700">Reports Due</div>
                <div className="mt-2 text-3xl font-bold text-red-700">{dueReports}</div>
              </div>
              <div className="rounded-xl border border-amber-200 bg-amber-50 p-5">
                <div className="text-xs font-semibold uppercase tracking-wide text-amber-700">Projects At Risk</div>
                <div className="mt-2 text-3xl font-bold text-amber-700">{riskProjects.length}</div>
              </div>
              <div className="rounded-xl border border-blue-200 bg-blue-50 p-5">
                <div className="text-xs font-semibold uppercase tracking-wide text-blue-700">Open Actions</div>
                <div className="mt-2 text-3xl font-bold text-blue-700">{openActionTickets}</div>
              </div>
            </div>

            <div id="pm-priority" className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
              <h3 className="text-lg font-semibold text-slate-900">Priority Queue</h3>
              <p className="mt-1 text-sm text-slate-600">Items that need immediate manager follow-up.</p>
              <div className="mt-4 space-y-3">
                {projectAlerts.length === 0 ? (
                  <p className="text-sm text-slate-600">No critical exceptions right now.</p>
                ) : (
                  projectAlerts.slice(0, 6).map((alert, index) => (
                    <div key={`${alert.project}-${alert.issue}-${index}`} className="rounded-lg border border-slate-200 p-4">
                      <div className="flex flex-wrap items-center justify-between gap-2">
                        <div className="text-base font-semibold text-slate-900">{alert.project}</div>
                        <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-700">{alert.severity}</span>
                      </div>
                      <div className="mt-1 text-sm text-slate-700">{alert.issue}</div>
                      <div className="mt-2 text-sm text-slate-600">Owner: {alert.responsible} • Action: {alert.action}</div>
                    </div>
                  ))
                )}
              </div>
            ...
            <div id="pm-projects" className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                ...
                <table className="min-w-full text-left text-sm text-slate-700">
                  <thead className="bg-slate-50 text-xs uppercase tracking-wide text-slate-600">
                    <tr>
                      ...
                      <th className="px-3 py-3">Open Issues</th>
                      <th className="px-3 py-3">Daily Report</th>
                    </tr>
                  </thead>
                  <tbody>
                    {projects.slice(0, 8).map((project) => {
                      ...
                      return (
                        <tr key={project.id} className="border-t border-slate-200">
                          ...
                          <td className="px-3 py-3 capitalize">{project.status}</td>
                          <td className="px-3 py-3">{Number(projectProfitability?.profit_margin || 0).toFixed(1)}%</td>
                          <td className="px-3 py-3">{projectOpenIssues}</td>
                          <td className="px-3 py-3">{latestReport?.status || "Not Started"}</td>
                        </tr>
                      );
                    })}
                  </tbody>
            ...
            <div id="pm-actions" className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
              <h3 className="text-lg font-semibold text-slate-900">Fast Actions</h3>
              <p className="mt-1 text-sm text-slate-600">Go straight to the work instead of scrolling through every panel.</p>
              <div className="mt-4 flex flex-wrap gap-2">
                <Link href="/daily-production" className="rounded-lg bg-slate-800 px-4 py-2 text-sm font-semibold text-white hover:bg-slate-900">Review Daily Reports</Link>
                <Link href="/tickets" className="rounded-lg border border-slate-300 px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-100">Open Ticket Queue</Link>
                <button type="button" onClick={() => runProjectManagerAiReview("Identify Projects at Risk", "")}
                  className="rounded-lg border border-indigo-300 px-4 py-2 text-sm font-semibold text-indigo-700 hover:bg-indigo-50">
                  Run AI Risk Review
                </button>
                <button type="button" onClick={() => setProjectManagerActionDraft((prev) => ({ ...prev, title: "", description: "", notes: "" }))}
